...
module.exports = function(grunt) {
  ...
  grunt.registerMultiTask('esteCoffee', 'Compile CoffeeScript files into JavaScript', function() {
  ...
  var writeFile = function (path, output) {
    if (output.length < 1) {
      ...
      // fix compiled output for Google Closure
      output = coffee2closure.fix(output);
      grunt.file.write(path, output);
      grunt.log.writeln('File ' + path[consoleColor] + ' created.');
    }
  };
};
